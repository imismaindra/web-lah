                ...
                <div class="w-full space-y-6 lg:w-80">
                    ...
                    <div class="rounded-2xl border border-stone-200/60 bg-white p-5 dark:border-white/[0.06] dark:bg-[#171716]">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex items-center gap-2 rounded-xl bg-stone-50 px-3 py-2.5 text-xs text-stone-400 dark:bg-white/[0.02] dark:text-stone-500">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach ($topiks as $topik)
                                <label class="flex cursor-pointer items-center gap-1.5 rounded-lg border border-stone-200 bg-stone-50 px-2.5 py-1.5 text-xs text-stone-600 transition hover:border-stone-300 dark:border-white/10 dark:bg-white/[0.03] dark:text-stone-300 dark:hover:border-white/20">
                                    <input type="checkbox" name="topik_ids[]" value="{{ $topik->id }}" class="h-3.5 w-3.5 accent-[#1e3a5f] dark:accent-[#5b9bd5]" {{ in_array($topik->id, old('topik_ids', $artikel->topiks->pluck('id')->all())) ? 'checked' : '' }}>
                                    {{ $topik->nama }}
                                </label>
                            @endforeach
                        </div>
                    ...
                    <div class="rounded-2xl border border-stone-200/60 bg-white p-5 dark:border-white/[0.06] dark:bg-[#171716]">
                        ...
                        @if ($artikel->gambar)
                            <div id="current-image" class="mb-3">
                                <img src="{{ asset('storage/' . $artikel->gambar) }}" alt="{{ $artikel->judul }}" class="w-full rounded-xl object-cover">
                            </div>
                        @endif
                    ...
                    <div class="rounded-2xl border border-stone-200/60 bg-white p-5 dark:border-white/[0.06] dark:bg-[#171716]">
                        ...
                        <p class="mt-1.5 text-right text-[10px] text-stone-300 dark:text-stone-600"><span id="ringkasan-count">{{ strlen(old('ringkasan', $artikel->ringkasan ?? '')) }}</span>/1000</p>
                        @error('ringkasan')
                            <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    {{-- Info --}}
                    <div class="rounded-2xl border border-stone-200/60 bg-white p-5 dark:border-white/[0.06] dark:bg-[#171716]">
                        <h3 class="mb-4 text-sm font-bold text-stone-800 dark:text-stone-200">Informasi</h3>
                        <div class="space-y-2 text-xs text-stone-400 dark:text-stone-500">
                            <div class="flex justify-between">
                                <span>Slug</span>
                                <code class="rounded bg-stone-100 px-1.5 py-0.5 text-[10px] dark:bg-white/[0.05]">{{ $artikel->slug }}</code>
                            </div>
                            <div class="flex justify-between">
                                <span>Dilihat</span>
                                <span class="font-semibold text-stone-600 dark:text-stone-300">{{ number_format($artikel->views) }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span>Dibuat</span>
