...
@section('title', 'Era')
@section('page-title', 'Era')

@section('content')
    {{-- Header --}}
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <p class="text-sm text-stone-500 dark:text-stone-400">Kelola era di halaman beranda</p>
        </div>
        <a href="{{ route('admin.era.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-[#1e3a5f] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[#16304a] dark:bg-[#5b9bd5] dark:text-[#0f0f0e] dark:hover:bg-[#7ab3e0]">
    ...
    <div class="mt-6 overflow-hidden rounded-2xl border border-stone-200/60 bg-white dark:border-white/[0.06] dark:bg-[#171716]">
        @if ($eras->isEmpty())
            <div class="p-12 text-center">
                <svg class="mx-auto h-12 w-12 text-stone-300 dark:text-stone-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                <p class="mt-3 text-sm text-stone-500 dark:text-stone-400">Belum ada era</p>
                <a href="{{ route('admin.era.create') }}" class="mt-3 inline-block text-sm font-semibold text-[#1e3a5f] dark:text-[#5b9bd5]">Tambah era baru &rarr;</a>
            </div>
        @else
            <table class="w-full text-left text-sm">
                <thead>
                    <tr class="border-b border-stone-100 dark:border-white/[0.04]">
                        ...
                        <th class="hidden px-5 py-3.5 text-xs font-bold uppercase tracking-wider text-stone-400 dark:text-stone-500 sm:table-cell">Periode</th>
                ...
                <tbody class="divide-y divide-stone-100 dark:divide-white/[0.04]">
                    @foreach ($eras as $era)
                        <tr class="group transition hover:bg-stone-50/50 dark:hover:bg-white/[0.01]">
                            <td class="px-5 py-4">
                                <div>
                                    <p class="font-semibold text-stone-800 dark:text-stone-200">{{ $era->nama }}</p>
                                    <code class="mt-0.5 inline-block rounded-md bg-stone-100 px-1.5 py-0.5 text-xs text-stone-500 dark:bg-white/[0.05] dark:text-stone-400">{{ $era->slug }}</code>
                                </div>
                            </td>
                            <td class="hidden px-5 py-4 sm:table-cell">
                                <span class="text-xs text-stone-500 dark:text-stone-400">{{ $era->periode ?: '—' }}</span>
                            </td>
                            <td class="hidden px-5 py-4 md:table-cell">
                                <span class="text-xs text-stone-400 dark:text-stone-500">{{ $era->urutan }}</span>
                            </td>
                            <td class="hidden px-5 py-4 lg:table-cell">
                                @if ($era->gambar)
                                    <img src="{{ asset('storage/' . $era->gambar) }}" alt="{{ $era->nama }}" class="h-10 w-16 rounded-lg object-cover">
                                @else
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-1">
                                    <a href="{{ route('admin.era.edit', $era) }}" class="rounded-lg p-2 text-stone-400 transition hover:bg-stone-100 hover:text-[#1e3a5f] dark:hover:bg-white/[0.05] dark:hover:text-[#5b9bd5]" title="Edit">
                                    ...
                                    <form method="POST" action="{{ route('admin.era.destroy', $era) }}" onsubmit="return confirm('Yakin ingin menghapus era ini?')" class="inline">
                                        @csrf
                                        @method('DELETE')
