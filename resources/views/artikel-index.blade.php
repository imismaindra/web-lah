...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Semua Artikel — {{ config('app.name', 'Look at History') }}</title>

        @include('partials.seo', [
            'title' => 'Semua Artikel',
            'description' => 'Jelajahi semua artikel sejarah dunia di Look at History: peristiwa, tokoh, dan peradaban dari berbagai era.',
            'image' => asset('logo_LAH.jpg'),
    ...
    <body class="bg-[#faf9f7] dark:bg-[#0f0f0e] text-[#171717] dark:text-[#e5e5e3] font-sans antialiased">
        <header class="sticky top-0 z-20 border-b border-stone-200/80 dark:border-white/[0.06] bg-[#faf9f7]/80 dark:bg-[#0f0f0e]/80 backdrop-blur-xl">
            <nav class="mx-auto flex max-w-[1400px] items-center justify-between px-5 py-4 sm:px-8">
                ...
                <div class="hidden items-center gap-1 text-sm font-medium sm:flex">
                    ...
                    <a href="{{ route('artikel.index') }}" class="rounded-lg bg-stone-900 px-3 py-1.5 text-white dark:bg-white dark:text-stone-900">Artikel</a>
        ...
        <main class="mx-auto max-w-[1400px] px-5 sm:px-8">
            ...
            <section class="pt-12 pb-8 sm:pt-16">
                <p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-stone-400 dark:text-stone-500">Artikel</p>
                <h1 class="mt-3 font-serif text-3xl font-bold leading-[1.1] tracking-tight sm:text-4xl lg:text-5xl">Semua Artikel</h1>
                <p class="mt-4 max-w-xl text-base leading-relaxed text-stone-500 dark:text-stone-400">
                    Peristiwa, tokoh, dan peradaban masa lalu — disusun ringkas, akurat, dan mudah dipahami.
                </p>
            </section>
                    ...
                    @foreach ($artikels as $artikel)
                        <a href="{{ route('artikel.show', $artikel) }}" class="group overflow-hidden rounded-2xl border border-stone-200/60 bg-white dark:border-white/[0.06] dark:bg-[#171716]">
                            <div class="relative overflow-hidden bg-stone-100 dark:bg-stone-800/50">
                                ...
                                @else
                                    <img src="https://picsum.photos/seed/artikel-index-{{ $artikel->id }}/600/400" alt="{{ $artikel->judul }}" class="h-44 w-full object-cover transition duration-700 group-hover:scale-105" loading="lazy">
                                @endif
                            </div>
                            <div class="p-5">
