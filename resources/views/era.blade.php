    ...
    <body class="bg-[#faf9f7] dark:bg-[#0f0f0e] text-[#171717] dark:text-[#e5e5e3] font-sans antialiased">
        ...
        <main class="mx-auto max-w-[1400px] px-5 sm:px-8">
            ...
            <section class="pt-12 pb-8 sm:pt-16">
                ...
                <p class="mt-8 text-[11px] font-semibold uppercase tracking-[0.18em] text-stone-400 dark:text-stone-500">Era &middot; {{ $era->periode }}</p>
                <h1 class="mt-3 font-serif text-3xl font-bold leading-[1.1] tracking-tight sm:text-4xl lg:text-5xl">{{ $era->nama }}</h1>
                @if ($era->periode)
                    <p class="mt-4 max-w-xl text-base leading-relaxed text-stone-500 dark:text-stone-400">{{ $era->periode }}</p>
                @endif
            </section>
            ...
            @if ($artikels->isEmpty())
                <div class="rounded-2xl border border-dashed border-stone-200 bg-white p-12 text-center dark:border-white/[0.06] dark:bg-[#171716]">
                    ...
                    <p class="mt-3 text-sm text-stone-500 dark:text-stone-400">Belum ada artikel pada era ini.</p>
                </div>
            @else
                    ...
                    @foreach ($artikels as $artikel)
                        <a href="{{ route('artikel.show', $artikel) }}" class="group overflow-hidden rounded-2xl border border-stone-200/60 bg-white dark:border-white/[0.06] dark:bg-[#171716]">
                            <div class="relative overflow-hidden bg-stone-100 dark:bg-stone-800/50">
                                ...
                                @else
                                    <img src="https://picsum.photos/seed/era-{{ $artikel->id }}/600/400" alt="{{ $artikel->judul }}" class="h-44 w-full object-cover transition duration-700 group-hover:scale-105" loading="lazy">
                                @endif
                            </div>
                            <div class="p-5">
